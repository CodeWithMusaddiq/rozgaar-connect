...
/**
 * AddJobPage - Connected to backend, creates real jobs
 */
const AddJobPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      setFormError(result.message || "Failed to create job");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6] py-6 sm:py-10">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-[#0F172A]">Post a New Job</h1>
          <p className="text-[#374151] text-sm mt-1">Fill in the details below to create a new job listing</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm p-6 sm:p-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            <button
              type="submit"
              disabled={loading}
              className="flex-1 bg-[#22C55E] hover:bg-[#16a34a] disabled:bg-gray-400 text-white py-3.5 rounded-xl font-semibold text-sm transition-colors flex items-center justify-center gap-2"
            >
              {loading ? (
